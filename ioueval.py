...
from borderMask import borderMask
...
class iouEval:
  ...
  def reset(self):
    self.conf_matrix = torch.zeros(
        (self.n_classes, self.n_classes), device=self.device).type(torch.DoubleTensor)#long() #before when it wasn;t double there would happen overflow corrupting the calculations
    self.ones = None
  ...
  def addBatch(self, x, y):  # x=preds, y=targets
    ...
    if self.ones is None or self.last_scan_size != idxs.shape[-1]:
      self.ones = torch.ones((idxs.shape[-1]), device=self.device).type(torch.DoubleTensor)#long()
  ...
  def getStats(self):
    ...
    # get the clean stats
    tp = conf.diag()
    fp = conf.sum(dim=1) - tp # for that ground truth class how many wrong predictions
    fn = conf.sum(dim=0) - tp #for that predictions how many wrong times
    return tp, fp, fn
